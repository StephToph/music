	...
	public function index($param1='', $param2='', $param3='') {
			...
			if($param2 == 'delete') {
				...
				if($_POST){
					...
					if($_FILES['pics']['name']){
						...
						$path = 'assets/img/artist';
						
						if (!is_dir($path))
							mkdir($path, 0755);
			
						$pathMain = './assets/img/artist';
						if (!is_dir($pathMain))
							mkdir($pathMain, 0755);
		...
		if($param1 == 'list') {
			...
			foreach ($list as $item) {
				...
				// edit 
				$edit_btn = '
					<a class="text-primary pop" href="javascript:;" pageTitle="Manage Record" pageName="'.base_url('artist/index/manage/edit/'.$id).'" pageSize="modal-sm" data-toggle="tooltip" title="EDIT">
				...
				// delete 
				$del_btn = '
					<a class="text-danger pop" href="javascript:;" pageTitle="Delete Record" pageName="'.base_url('artist/index/manage/delete/'.$id).'" pageSize="modal-sm" data-toggle="tooltip" title="DELETE">
		...
		if($param1 == 'manage') {
			$this->load->view('artist/manage_form', $data);
		} else {
			// for datatable
			$data['table_rec'] = 'artist/index/list'; // ajax table url
			$data['order_sort'] = '1, "desc"'; // default ordering (0, 'asc')
			$data['no_sort'] = '0,3'; // sort disable columns (1,3,5)

			//$data['user'] = $account_name;
			$data['title'] = 'Manage Artist | '.app_name;
			$data['page_active'] = 'artist';
			$this->load->view('designs/header', $data);
			$this->load->view('artist/manage', $data);
