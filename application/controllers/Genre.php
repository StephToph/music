	...
	public function index($param1='', $param2='', $param3='') {
		...
		if($param1 == 'list') {
			...
			foreach ($list as $item) {
				...
				// edit 
				$edit_btn = '
					<a class="text-primary pop" href="javascript:;" pageTitle="Manage Record" pageName="'.base_url('genre/index/manage/edit/'.$id).'" pageSize="modal-sm" data-toggle="tooltip" title="EDIT">
				...
				// delete 
				$del_btn = '
					<a class="text-danger pop" href="javascript:;" pageTitle="Delete Record" pageName="'.base_url('genre/index/manage/delete/'.$id).'" pageSize="modal-sm" data-toggle="tooltip" title="DELETE">
		...
		if($param1 == 'manage') {
			$this->load->view('genre/manage_form', $data);
		} else {
			// for datatable
			$data['table_rec'] = 'genre/index/list'; // ajax table url
			$data['order_sort'] = '0, "desc"'; // default ordering (0, 'asc')
			$data['no_sort'] = '2'; // sort disable columns (1,3,5)

			//$data['user'] = $account_name;
			$data['title'] = 'Manage Genre | '.app_name;
			$data['page_active'] = 'genre';
			$this->load->view('designs/header', $data);
			$this->load->view('genre/manage', $data);
